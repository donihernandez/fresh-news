<template>
  <v-container fluid>
    <v-row align="center" justify="center" class="mt-3 mb-3 mx-auto">
      <h1 class="text-h1 font-weight-bold">Sports News</h1>
    </v-row>
...
export default {
  ...
  methods: {
    async getNews () {
      const data = await this.$axios.$get(`/top-headlines?country=us&category=sports&pageSize=${this.pageSize}&apiKey=${process.env.API_SECRET}`)
